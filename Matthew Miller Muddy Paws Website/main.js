...
function makeTable() {
    ...
    if (localStorage.storageCart !== undefined) {
        ...
        for (var i = 0; i < rows; i++) {   //for loop to create a row then put a cell in for each column
            ...
            cell2.innerHTML = cartInStorage[i].title;
            cell3.innerHTML = '<br> <br> <br> <br> <br>' + "<b>size: </b> " + cartInStorage[i].size + '<br> <br>' + '<b>color: </b>' + cartInStorage[i].color + '<br> <br> <br>' +  "<button class= 'remove'> Remove </button>";
            cell4.innerHTML = cartInStorage[i].price;
            cell5.innerHTML = quantSelect; //add selector to cell for selecting quantity
        }
        ...
        head3.innerHTML = '<th></th>';
        head4.innerHTML = '<th>Price</th>';
        head5.innerHTML = '<th>Quantity</th>';
    }
    else {
